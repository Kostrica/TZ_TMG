...
countries.forEach(function (country, code) {
  ...
  $("<option>", { text: `${country} (${codes[code]})`, val: codes[code] }).appendTo($select);//added a mask to the country
});   
...
// getting Bearer Token from REST-API and go to thanks.html page
...
const logIn = () => {
  return fetch('https://api.topmediagroups.com/login', {
    method: 'POST',
    headers: {
...
const authentication = (token) => {
  return fetch('https://api.topmediagroups.com/api/v1/tasks', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json;charset=utf-8',
      'Authorization': token,
...
const authorization = () => {
  return fetch('https://api.topmediagroups.com/api/v1/users', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json;charset=utf-8'
    },
  ...
  .then(({ code }) => {
    if (code === 201) {
      logIn()
        ...
        .then(({ code, message }) => {
          if (code === 200) {
            authentication(message.Authorization)
              .then(res => res.json())
              .then(({ code }) => {
                if (code === 201) {
...
// document.querySelector('form').addEventListener('submit', e => {
  ...
  $(".sign-up-button").on('click', function() {
    ...
    if (onSubmit) {
      ...
      userLog.email = myEmail;
      userLog.password = myPass;
...
function valid(event) {
  if (event.key === 'Backspace' && $('#selectCountry').val() === $('#code').val()) {//added the ability to edit the phone number
